...
class FixedSizeOrderedDict(OrderedDict):
    ...
    def generate_unique_id(self) -> int:
        ...
        return unique_id
     # Add these pickle methods
    def __getstate__(self):
        state = super().__getstate__()
        state.update({
            'max_size': self.max_size,
            '_id_set': self._id_set,
            '_id_counter': self._id_counter
        })
        return state

    def __setstate__(self, state):
        self.max_size = state.pop('max_size')
        self._id_set = state.pop('_id_set')
        self._id_counter = state.pop('_id_counter')
...
class CachedAudioDataset(Dataset):
    ...
    def __getstate__(self):
        ...
        # Remove the cache and generator as they can't be pickled
        state['cache'] = None
        state['_cache_read_pos_generator'] = None
        return state

    def __setstate__(self, state):
        self.__dict__.update(state)
        # Reinitialize cache in the new process
        self._init_cache()
